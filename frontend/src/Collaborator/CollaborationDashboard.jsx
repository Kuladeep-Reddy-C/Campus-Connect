...
const CollaborationDashboard = () => {
    ...
    const fetchProjectsAndForms = async () => {
      try {
        const [projectRes, formRes] = await Promise.all([
          ...
          fetch("http://localhost:5000/api/join-forms"),
        ]);
  ...
  const openRequestsModal = (projectId) => {
    const filtered = joinForms.filter((form) => form.projectId === projectId);
  ...
  const handleRequestAction = async (requestId, status, projectId) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/join-forms/${requestId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleJoinSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/join-forms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
